...
export function loadSample(): Promise<string> {
...
export function Preview({ settings, renderer, source, real = false, onModeChange, available = false, disabled = false }: {
  ...
  const unsupportedReal = real && (settings.shapeMode === "custom" || source?.shape.shapeMode === "custom");
  useEffect(() => {
    let cancelled = false;
    setRendering(true);
    const timeout = window.setTimeout(async () => {
      try {
        if (unsupportedReal) { setFrame(null); setError(""); return; }
        ...
        const shape = source?.shape ?? {
          ...
          width: circle ? 150 : 330, height: 150, rotation: 0, adjustment: 0.2, shapeMode: circle ? "circle" as const : "roundedRectangle" as const,
        ...
        const rendered = source?.rendered && JSON.stringify(source.settings) === JSON.stringify(settings) ? source.rendered : await renderer.render({ slideImageBase64: background, shape,
          settings: { ...settings, shapeMode: source ? shape.shapeMode ?? settings.shapeMode : shape.shapeMode ?? "roundedRectangle", outputScale: source ? settings.outputScale : 2 } });
        if (!cancelled) {
  ...
  }, [renderer, settings, source, real, unsupportedReal]);
  ...
  return <div className={`preview ${unsupportedReal ? "unsupported-real-preview" : ""}`} aria-label="液态玻璃预览" aria-busy={rendering}
    style={{
    ...
    <div className="preview-switch" role="group" aria-label="预览来源">
      ...
      <button type="button" aria-pressed={!real} disabled={disabled} title="使用固定背景查看材质，不修改幻灯片" onClick={() => onModeChange(false)}>示例</button>
      <button type="button" aria-pressed={real} disabled={disabled || !available} title="读取所选图形后方的幻灯片背景；应用效果才会写入 PPT" onClick={() => onModeChange(true)}>真实</button>
    </div>
    {unsupportedReal && <div className="preview-unsupported" role="status">
      <div className="preview-unsupported-card"><strong>真实预览暂不支持</strong><span>当前自由轮廓将使用圆角矩形示例展示；应用效果仍支持此图形。</span></div>
    </div>}
    {frame && bounds && <img className="sample-material" src={frame.image}
      alt={frame.real ? "当前选区与真实背景的玻璃效果" : "示例背景上的玻璃效果"} style={{
        ...
        maskImage: frame.shape.maskBase64 ? `url("${frame.shape.maskBase64.startsWith("data:") ? frame.shape.maskBase64 : `data:image/png;base64,${frame.shape.maskBase64}`}")` : undefined,
        maskSize: "100% 100%",
        maskRepeat: "no-repeat",
        filter: `brightness(${1 + Math.max(-100, Math.min(100, settings.brightness ?? 0)) / 100})`,
        borderRadius: frame.shape.shapeMode === "custom" ? "0" : (frame.shape.shapeMode ?? settings.shapeMode) === "circle" ? "50%" : `${(frame.shape.adjustment ?? 0.16667) * Math.min(frame.shape.width, frame.shape.height) / frame.shape.width * 100}% / ${(frame.shape.adjustment ?? 0.16667) * Math.min(frame.shape.width, frame.shape.height) / frame.shape.height * 100}%`
      }} />}
