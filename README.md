...
LiquidSlide 是一个 Windows 桌面 PowerPoint 加载项，为可填充的自选图形和自由曲线添加液态玻璃效果。选择图形、预览材质，再点击“应用效果”，就能把背景的色彩、折射和柔雾融入幻灯片，同时保留原图形与文字。
...
当前版本 **1.0.0** · 免费开源 · MIT 许可证

## 实际效果

![LiquidSlide 在 PowerPoint 中的四种玻璃效果与蓝紫色任务面板](assets/演示效果.png)

与官网一致的 PowerPoint 实际截图，由维护者提供。左侧展示通透玻璃、珍珠玻璃、曜黑玻璃和磨砂玻璃等应用效果，右侧为新版蓝紫白色任务面板，截图中的材质预览处于“示例”模式。
...
| 材质 | 效果与用途 |
| --- | --- |
| 通透玻璃（Clear Glass） | 清晰折射与细腻高光，保留背景细节 |
| 珍珠玻璃（Pearl Glass） | 带白色的半透明质感，适合搭配深色文字 |
| 曜黑玻璃（Obsidian Glass） | 深色半透明质感，适合搭配浅色文字 |
| 磨砂玻璃（Frosted Glass） | 柔化复杂背景，让前景内容更突出 |
...
- **保留原图形**：支持单选可填充的自选图形和自由曲线，生成效果后填回原对象，保留文字
...
1. 前往 [GitHub Releases](https://github.com/ZiChen-Whisper/LiquidSlide/releases/latest)，下载 `LiquidSlide-1.0.0-windows-x64-setup.exe`
...
```powershell
Get-FileHash .\LiquidSlide-1.0.0-windows-x64-setup.exe -Algorithm SHA256
```

## 使用方法

**快速应用**：单选自选图形或自由曲线，直接点击顶部 LiquidSlide 选项卡中的任一材质按钮，无需打开面板。

需要预览和自定义参数时：

1. 在幻灯片中单选一个自选图形或自由曲线，打开 **LiquidSlide → 打开面板**
...
- “我的预设”将当前材质和亮度另存到本机，支持同名另存，保留内置及已有预设，并同步到其他已打开的面板。
- 自选图形、自由曲线使用实际轮廓生成折射和高光，覆盖椭圆、多边形、箭头、星形、凹形和带孔轮廓；圆形和圆角矩形沿用原渲染。组合、图片、表格、线条需先转换为可填充图形。
- 整体亮度范围为 -100 到 100，0 为默认；应用时设置 PowerPoint `PictureFormat.Brightness`，面板亮度预览为近似效果。
- 安装向导改为中文 + English 同时显示。窄面板在 280 px 以下使用单列预设，顶部显示 PowerPoint 连接状态。
- 通透玻璃主高光与反向高光均为 0.8；珍珠玻璃和曜黑玻璃背景模糊均为 3；磨砂玻璃保留较柔和的 0.65 高光。
- 已保存到幻灯片中的旧材质参数保持原值；重新选择内置预设即可使用新参数。

本轮仅编译和打包，按要求未运行测试，也未进行 PowerPoint 或安装程序实机验收。

### 任意轮廓渲染修订

已移除独立的简化 ContourRenderer。任意形状现在将实际轮廓转换为内外距离场，
接入项目内维护的 liquid-dom 0.1.1 修改版，复用原有折射、高光、色散及模糊流程。
源码与补丁说明见 `vendor/liquid-dom/README.md`。圆形和圆角矩形保留原距离公式。
修正了 PPT 轮廓导出模式和尺寸换算；并非把矩形玻璃裁成目标形状。
此修订仍待 PowerPoint 实际验收。

### 对齐修订 3：运行版本一致性

生产脚本使用内容哈希文件名，面板页面地址按 HTML 内容哈希更新，避免升级后复用旧缓存。
读取选区前检查宿主与前端协议修订号，拒绝新旧组件混用。面板显示“对齐修订 3”。
此次针对当前三角形做了临时副本的轮廓提取和独立 WebGPU 诊断，当前源码结果对齐；
实际 PPT 现有填充已出现偏移。缓存是否为该现象的唯一原因尚未确认，完整宿主流程仍待验收。


### 矢量轮廓修订 4

任意形状改用 PowerPoint 原生 SVG 轮廓及显式坐标框，替代 PNG 透明边界校准。
坐标框与原图形同尺寸，保留原有空白区域；移除导出时残留的阴影副本，
再交给原来的 liquid-dom 距离场折射管线。需 Office 支持 SVG 图形导出。
前端支持 SVG 遮罩与解码，宿主协议同步为 `contour-vector-4`。
此前的独立 WebGPU、WebView2 和临时文档完整面板流程均得到对齐结果，
但未复现用户原窗口的错位，因此此修订不能宣称根因已确认或原窗口已修复。
仍需用户在实际 PowerPoint 面板重新应用后验收。
